                ...
                <div class="card-body">
                    <form action="{{ route('major.update', $major->id) }}" method="post">
                        ...
                        <div class="form-group mb-2">
                            ...
                            <input type="text" name="name" class="form-control rounded-0 @error('name') is-invalid @enderror" id="name" placeholder="Input Name..." value="{{ old('name', $major->name) }}">
                            @error('name')
                        ...
                        <div class="form-group mb-2">
                            ...
                            <select class=" @error('department_id') is-invalid @enderror select2" id="department_id" name="department_id" style="width: 100%;">
                                <option selected disabled>Choose Department...</option>
                                @foreach($departments as $department)
                                    <option value="{{ $department->id }}" {{ old('department', $major->department_id) == $department->id ? 'selected' : '' }}>{{ Str::title($department->name) }}</option>
                                @endforeach
                            </select>
                            @error('department_id')
                                <small class="text-danger">{{ $message }}</small>
