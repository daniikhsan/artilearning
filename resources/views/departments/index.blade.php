...
<div class="content-header">
    <div class="container-fluid">
        <div class="row mb-2">
            ...
            <div class="col-md-6">
                <a href="{{ route('department.create') }}" class="btn btn-outline-primary float-right"><i class="fas fa-plus mr-2"></i> Add Department</a>
            </div>
        </div><!-- /.row -->
                            ...
                            <tr>
                                ...
                                <td>
                                    @foreach($department->majors as $major)
                                        {{ $loop->iteration }}. {{ $major->name }} <br>
                                    @endforeach
                                </td>
                                <td class="text-center">
                                    <div class="btn btn-toolbar justify-content-center">
                                        <a href="{{ route('department.edit', $department->id) }}" class="btn btn-sm btn-primary mr-2"><i class="fa fa-pen"></i></a>
                                        <form action="{{ route('department.destroy', $department->id) }}" method="post" onsubmit=" return confirm('Are you sure want to delete this item?')">
                                            @csrf
                                            @method('DELETE')
                                            <button class="btn btn-sm btn-danger {{ $department->role == 'admin' ? 'disabled' : '' }}"><i class="fa fa-trash"></i></button>
                                        </form>
                                    </div>
                                </td>
                            </tr>
