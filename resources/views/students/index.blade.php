...
<div class="content-header">
    <div class="container-fluid">
        <div class="row mb-2">
            ...
            <div class="col-md-6">
                <a href="{{ route('student.create') }}" class="btn btn-outline-primary float-right"><i class="fas fa-plus mr-2"></i> Add Student</a>
            </div>
        </div><!-- /.row -->
                    ...
                    <tbody>
                        @foreach($students as $student)
                            <tr>
                                ...
                                <td class="text-center">{{ Str::title($student->role) }}</td>
                                <td class="text-center">
                                    <div class="btn btn-toolbar justify-content-center">
                                        <button class="btn btn-sm btn-default mr-2" data-toggle="modal" data-target="#modal-default-{{ $student->id }}"><i class="fa fa-eye"></i></button>
                                        <a href="{{ route('student.edit', $student->id) }}" class="btn btn-sm btn-primary mr-2"><i class="fa fa-pen"></i></a>
                                        <form action="{{ route('student.destroy', $student->id) }}" method="post" onsubmit=" return confirm('Are you sure want to delete this item?')">
                                            @csrf
                                            @method('DELETE')
                                            <button class="btn btn-sm btn-danger {{ $student->role == 'admin' ? 'disabled' : '' }}"><i class="fa fa-trash"></i></button>
                                        </form>
                                    </div>
                                </td>
                            </tr>
                            ...
                            <div class="modal fade" id="modal-default-{{$student->id}}">
                                <div class="modal-dialog">
                                    ...
                                    <div class="modal-body">
                                        <div class="row">
                                            <div class="col-3">
                                                <img src="{{ url($student->avatar) }}" alt="{{ $student->name }}" width="100" class="mb-3">
                                            </div>
                                            <div class="col-9">
                                                <small class="m-0">Name</small>
                                                <p class="mb-2">{{ $student->name }}</p>
                                                <small class="m-0">Email</small>
                                                <p class="mb-2"><i>{{ $student->email }}</i></p>
                                                <small class="m-0">Role</small>
                                                <p class="mb-2"><i>{{ Str::title($student->role) }}</i></p>
                                                <small class="m-0">Major/Department</small>
                                                <p class="mb-2"><i>{{ $student->major->name }}/{{ $student->major->department->name }}</i></p>
                                                <small class="m-0">Gender</small>
                                                <p class="mb-2"><i>{{ $student->gender }}</i></p>
                                                <small class="m-0">Address</small>
                                                <p class="mb-2"><i>{{ $student->address }}</i></p>
                                            </div>
                                        </div>
                                    </div>
                                    <div class="modal-footer justify-content-between">
                                        <div>
                                            <a href="{{ route('student.edit', $student->id) }}" class="btn btn-sm btn-primary mr-2"><i class="fa fa-pen"></i> Edit</a>
                                            <form action="{{ route('student.destroy', $student->id) }}" method="post" onsubmit=" return confirm('Are you sure want to delete this item?')" style="display:inline;">
                                                @csrf
                                                @method('DELETE')
                                                <button class="btn btn-sm btn-danger {{ $student->role == 'admin' ? 'disabled' : '' }}"><i class="fa fa-trash"></i> Delete</button>
                                            </form>
                                        </div>
